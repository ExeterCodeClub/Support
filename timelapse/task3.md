# Task 3 Resources
**This document contains the resources you'll need to get started with making the video**

**Aim:** Your goal is to write a program that can convert a set of images into video format.
...
## Summary of aims
- find the folder filled with timelapse images and copy them for safe keeping
- rename the timelapse images into a specific format for `ffmpeg` to understand
- use ffmpeg to turn the timelapse images into a video
...
```bash
cd <location of task3-img>
```

which will point your terminal inside the folder task3-img. Then open all the images in a browser by typing 

```bash
<browser name> *
```

Where \<browser\> is probably chromium, firefox, midori, or something else. If you fish around in the menus you might find the name of a browser that's on the pi.
...
```bash
mv <best filename> image-000001.png
```
...
for image in sortedlist; do
    ...
    mv $image image-$imagenumber.png
...
These files are stored inside the testimages folder. That list of names gets stored in the variable `imagelist`. You can try running `list=$( ls )` by hand in another terminal, then `echo $list` to see that list printed out. `ls` on its own just lists the current directory.  The bits that say `$( command )` mean run this command in the terminal and give back the output. Guess what the next line is doing, with `sortedlist=...`.
...
If that made no sense to you, ask for an explanation. Try running in the command line 

```bash
for a in 1 2 3;do; echo $a; sleep 1; done
```

maybe that'll help!


The line that says `filenumber=$( printf "%06.0f" $i )` first runs `printf ...` which turns the number stored in `i` into a six digit padded number, then that gets stored in the variable `filenumber`. We want the filenames to be of the format 'image-xxxxxx.png', where xxxxxx is a 6 digit number, so '1' becomes '000001'. We need to do this because ffmpeg likes them to be that way. 

Try typing `printf "%03.2f" 11.5` into the terminal, you can change the numbers between %...f to have a play.

Next we are renaming the current file the for loop is focusing on to the name "image-$filenumber.png", the tenth file will be called "image-000010.png", and so on...
...
Save the script as imagerename.sh in the same folder as imagereset.sh, then make the script executable. 

After checking that things look right, try running it.

Did it rename all of the files? 

## Step 3
All you need to do to see the final video is to generate the video using ffmpeg.

Navigate into the testimages folder and run the command 

```bash
ffmpeg -r 10 -i image-%06d.png -c:v libx264 -pix_fmt yuv420p -y output.mp4
```

Make sure that the segment that says `image-%06d.png` is exactly correct. 

When finished, you should be able to open the video file in a browser by typing `<browser name> output.mp4`.


Put the ffmpeg line inside a script and make it executable for future use (don't forget the `#!/bin/bash` at the start.)


You are now finished, all that remains is to collect the timelapse images and change some of the lines in your scripts to point at the folder that stores the new timelapse image files. 

Remember, copy the images to a safe place before operating on them.
